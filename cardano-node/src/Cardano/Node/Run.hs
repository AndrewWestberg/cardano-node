...
runNode loggingLayer npm@NodeCLI{protocolFiles} = do
    ...
    case viewmode of
      ...
      LiveView   -> do
        ...
        trsinactive <- case ncTraceConfig nc of
                         TracingOff -> return []
                         TracingOn traceConf -> return $ map (\(tName,f) -> (tName, f traceConf)) reqtrs

        unless (List.all (\(_, tracerOn) -> tracerOn == True) trsinactive) $ do
